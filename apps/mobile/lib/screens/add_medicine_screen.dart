...
class _AddMedicineScreenState extends State<AddMedicineScreen> {
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: _pendingDrugs.isEmpty || _isSaving ? null : _saveBatchMedicines,
            icon: _isSaving
                ? const SizedBox(
                    width: 20,
                    height: 20,
            ...
            label: Text(
              _pendingDrugs.isEmpty
                  ...
                  : 'Zapisz ${_pendingDrugs.length} lek${_pendingDrugs.length == 1 ? '' : _pendingDrugs.length < 5 ? 'i' : 'ów'}',
            ),
          ),
        ],
      ),
          ...
          children: [
            ...
            if (_pendingDrugs.length > 1)
              TextButton.icon(
                ...
                label: Text('Wyczyść', style: TextStyle(color: theme.colorScheme.error)),
  ...
  Widget _buildPendingDrugCard(PendingDrug drug, int index, ThemeData theme, bool isDark) {
    final timeStr = '${drug.addedAt.hour.toString().padLeft(2, '0')}:${drug.addedAt.minute.toString().padLeft(2, '0')}';
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: const Icon(LucideIcons.shieldCheck, size: 20, color: Colors.green),
          ),
          const SizedBox(width: 12),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Icon(LucideIcons.package, size: 12, color: theme.colorScheme.onSurfaceVariant),
  ...
  Future<void> _onRplMedicineSelected(RplSearchResult result) async {
    ...
    try {
      ...
      if (selection != null) {
        ...
        _log.info('Drug added to pending list. Total pending: ${_pendingDrugs.length}');
  ...
  Future<void> _toggleManualSection() async {
    ...
    if (_pendingDrugs.isNotEmpty) {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            ...
            'lek${_pendingDrugs.length == 1 ? '' : _pendingDrugs.length < 5 ? 'i' : 'ów'}. '
      ...
      _log.info('User confirmed discarding ${_pendingDrugs.length} pending drugs');
      return;
    }
  ...
  Future<void> _clearPendingDrugs() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Czy na pewno chcesz usunąć ${_pendingDrugs.length} lek(ów) z listy?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
